...
_SORT_HINT: str = (
    ...
    "FIELD selector is a CUSTOM COMBOBOX (input + adjacent button). Steps: "
    "  1. In the AXTree, find the BUTTON element right after the 'Order results by' input. "
    "  2. browser_click(bid=<button_bid>) — do NOT click the input itself (it will time out). "
    "  3. noop() to wait for the field list to appear. "
    "  4. browser_click(bid=<option_bid>) to select the desired field. "
    ...
    "After ALL sort fields are set: browser_click() the 'Run' button. "
...
_FILTER_HINT: str = (
    ...
    "FIELD selector is a CUSTOM COMBOBOX (input + adjacent button). Steps: "
    "  1. In the AXTree, find the BUTTON right after the 'Choose field' input. "
    "  2. browser_click(bid=<button_bid>) — do NOT click the input (it will time out). "
    "  3. noop(), then browser_click(bid=<option_bid>) to select the field. "
    "OPERATOR selector: same pattern — click the adjacent BUTTON, then click the operator option. "
    ...
    "After ALL conditions are fully set: browser_click() the 'Run' button. "
