...
logger = logging.getLogger("waa_cube.vm_backend.getters.misc")
...
def apply_rules_to_timeFormat(timeFormat: str, absoluteDay: datetime):
    ...
    timeFormat = timeFormat.replace(
        "{Month0D}", "0" + str(absoluteDay.month) if absoluteDay.month < 10 else str(absoluteDay.month), 1
    )
    ...
    timeFormat = timeFormat.replace(
        "{Day0D}", "0" + str(absoluteDay.day) if absoluteDay.day < 10 else str(absoluteDay.day), 1
    )
