...
| `webarena-verified` (Ph2+3) | ✅ done | `WebArenaVerifiedTaskMetadata` typed subclass with `sites`, `expected_action`, `intent_template_id`; `task_metadata.json` is shipped package resource; `install()`/`uninstall()` removed (base no-ops); `scripts/generate_task_metadata.py` for developer regeneration |
| `workarena` (Ph2+3) | ✅ done | `WorkArenaTaskMetadata` typed subclass with `level`, `in_human_curriculum`, `task_class_path`; `named_subsets` glob keys updated (`extra_info.level` → `level`); `_setup()` uses `tm.level`; `task.py` uses `metadata.task_class_path`; `install()`/`uninstall()` removed; `scripts/generate_task_metadata.py` added |
