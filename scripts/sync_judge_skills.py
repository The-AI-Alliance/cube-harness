...
JUDGE_USE_CASES_DIR = Path("src/cube_harness/analyze/judge/use_cases")
...
def _sync(*, repo_root: Path, dry_run: bool) -> list[Path]:
    ...
    use_cases_dir = (repo_root / JUDGE_USE_CASES_DIR).resolve()
...
def main(
    ...
    """Sync judge-skill symlinks. Idempotent."""
    changed = _sync(repo_root=repo_root, dry_run=dry_run)
    if not changed:
        typer.echo("All judge-skill symlinks are up to date.")
        return
