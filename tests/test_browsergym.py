...
class TestBrowsergymToolActionMethods:
    ...
    def _create_tool_with_mock_env(self) -> BrowsergymTool:
        ...
        config = BrowsergymConfig()
        tool = BrowsergymTool(config)
        tool.session = PlaywrightSession(playwright=MagicMock(), page=MagicMock(), context=MagicMock(), cdp_url="http://localhost:9222")
        return tool
...
class TestBrowsergymToolLifecycle:
    ...
    def test_close_handles_exception(self) -> None:
        ...
        tool.session = PlaywrightSession(playwright=MagicMock(), page=MagicMock(), context=mock_context, cdp_url="http://localhost:9222")
        tool._last_obs = {"some": "data"}
...
class TestBrowsergymToolCheckboxJsFallback:
    ...
    def _create_tool_with_mock_page(self) -> BrowsergymTool:
        ...
        tool.session = PlaywrightSession(playwright=MagicMock(), page=mock_page, context=MagicMock(), cdp_url="http://localhost:9222")
        tool._last_obs = {}
