...
class TestPlaywrightSession:
    ...
    def test_stop_logs_warning_on_context_close_error(self) -> None:
        ...
        session = PlaywrightSession(
            page=MagicMock(), context=mock_context, browser=MagicMock(), cdp_url="http://localhost:1234"
        )
...
class TestPlaywrightSessionConfig:
    ...
    def test_make_returns_playwright_session(self) -> None:
        ...
        with (
            ...
            patch("agentlab2.tools.browser_session._get_cdp_url", return_value="http://localhost:1234"),
        ):
    ...
    def test_make_cdp_url_comes_from_get_cdp_url(self) -> None:
        mock_pw = self._make_mock_pw()
        with (
            patch("agentlab2.tools.browser_session._get_global_playwright", return_value=mock_pw),
            patch("agentlab2.tools.browser_session._get_cdp_url", return_value="http://localhost:9876") as mock_get_cdp,
        ):
            session = PlaywrightSessionConfig().make()
        mock_get_cdp.assert_called_once()
