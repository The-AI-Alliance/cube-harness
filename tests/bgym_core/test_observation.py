...
def test_extract_axtree_simple() -> None:
    env = gym.make(
        "browsergym/openended",
        task_kwargs={"start_url": TEST_PAGE},
        headless=__HEADLESS,
...
def test_extract_axtree_multi_iframe() -> None:
    env = gym.make(
        "browsergym/openended",
...
def test_extract_dom_simple() -> None:
    env = gym.make(
        "browsergym/openended",
        task_kwargs={"start_url": TEST_PAGE},
        headless=__HEADLESS,
...
def test_extract_dom_multi_iframe() -> None:
    env = gym.make(
        "browsergym/openended",
        task_kwargs={"start_url": MULTI_IFRAME_URL},
        headless=__HEADLESS,
...
def test_simple_shadowdom() -> None:
    env = gym.make(
        "browsergym/openended",
...
def test_nested_shadowdom() -> None:
    env = gym.make(
        "browsergym/openended",
...
def test_dom_has_bids_no_aria(url: str) -> None:
    env = gym.make(
        "browsergym/openended",
...
def test_axtree_to_text_remove_redundant() -> None:
    env = gym.make(
        "browsergym/openended",
        task_kwargs={"start_url": TEST_PAGE_2},
        headless=__HEADLESS,
...
def test_basic_iframe_webpage() -> None:
    """
    In this simple unit test, we make sure the retrieved coordinates of a known element
    are correct, by verifing that the element is checked after clicking on it.
...
def test_extract_focused_element_bid_through_shadowdom() -> None:
    env = gym.make(
        "browsergym/openended",
        task_kwargs={"start_url": SHADOW_DOM_URL},
        headless=__HEADLESS,
