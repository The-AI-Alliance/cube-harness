"""
Experiment viewer for cube-harness.
...
def run_viewer(results_dir: Path, debug: bool = False, port: int | None = None, share: bool = False) -> None:
    ...
    # Local state instance - captured by closures below
    ...
    with gr.Blocks(theme=gr.themes.Soft(), css=_CSS, head=_SHORTCUT_JS) as demo:  # type: ignore
        ...
        with gr.Accordion("Help", open=False):
            gr.Markdown(
                """\
# cube-harness Experiment Viewer
...
def main():
    ...
    parser = argparse.ArgumentParser(description="cube-harness Experiment Viewer")
    parser.add_argument(
        "--results-dir",
        type=str,
        default=str(EXP_DIR),
        help="Path to results directory containing experiments (default: cube_harness.EXP_DIR)",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
