"""Context-file resolution for the judge.

The judge needs a small map of on-disk paths it can grep — typically the cube
package, the agent package, and the cube-harness source. That map lives in
`<experiment_dir>/judge_context.md` as a fenced ```paths block:
...
`_load_experiment_view` is also kept here because the judge prompt builder
