...
`ch-judge <experiment_dir>` (no subcommand) is also accepted — it dispatches
...
constructing `JudgeBatchConfig` directly with `synthesis_model=""` or
...
from cube_harness.analyze.judge.core import JudgeBatchConfig, judge_experiment
...
def run_cmd(
    ...
    judge_model: Annotated[
        str | None,
        typer.Option("--judge-model", help="Override the recipe's per-episode judge model."),
    ] = None,
    ...
    By default judges every eligible (unjudged) episode, runs the post-batch
    ...
    if judge_model is not None and judge_model != chosen_recipe.model:
        chosen_recipe = chosen_recipe.model_copy(update={"model": judge_model})
...
def main(argv: list[str] | None = None) -> int:
    ...
    `ch-judge <experiment_dir>` (no subcommand keyword) dispatches to `run`
