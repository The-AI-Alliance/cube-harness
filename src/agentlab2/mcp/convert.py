"""Conversion utilities between AgentLab2 types and MCP types."""
...
def action_schema_to_mcp_tool(schema: ActionSchema) -> Tool:
    """Convert an AgentLab2 ActionSchema to an MCP Tool definition.
...
def observation_to_mcp_content(obs: Observation) -> list[TextContent | ImageContent]:
    """Convert an AgentLab2 Observation to a list of MCP content items.
...
def _content_to_mcp(content: Content) -> TextContent | ImageContent:
    """Convert a single AgentLab2 Content item to an MCP content item."""
    data = content.data
