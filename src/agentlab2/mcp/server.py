"""MCP server that exposes AgentLab2 tools via the Model Context Protocol."""
...
class McpServer:
    """Adapts an AgentLab2 tool to serve its actions as MCP tools.
...
def main() -> None:
    """CLI entry point for al2-mcp-server."""
    import argparse

    parser = argparse.ArgumentParser(description="Start an AgentLab2 MCP server")
    parser.add_argument(
        "--tool-config",
        type=str,
        required=True,
        help="Python import path to a ToolConfig instance, e.g. 'agentlab2.tools.playwright.PlaywrightConfig'",
    )
    parser.add_argument("--transport", choices=["stdio", "sse"], default="stdio")
    parser.add_argument("--host", default="localhost")
    parser.add_argument("--port", type=int, default=8000)
    parser.add_argument("--server-name", default="agentlab2")
