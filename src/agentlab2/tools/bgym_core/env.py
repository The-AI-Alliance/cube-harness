...
def _try_to_extract_legacy_goal(goal: list) -> str:
    legacy_goal_strings = []
    for message in goal:
...
class BrowserEnv(gym.Env, ABC):
    ...
    def reset(self, seed: int | None = None, *args, **kwargs) -> tuple[dict, dict]:
        ...
        self.browser = pw.chromium.launch(
            ...
            slow_mo=slow_mo,
            args=args,
            ignore_default_args=["--hide-scrollbars"],  # otherwise the screenshot doesn't see the scrollbars
...
"""
    ...
    def _task_validate(self) -> tuple[float, bool, str | None, dict]:
    ...
    def _wait_for_user_message(self) -> None:
    ...
    def _activate_page_from_js(self, page: playwright.sync_api.Page) -> None:
        logger.debug(f"_activate_page_from_js(page) called, page={str(page)}")
        if not page.context == self.context:
            raise RuntimeError(f"Unexpected: activating a page that belongs to a different browser context ({page}).")
    ...
    def _active_page_check(self) -> None:
        ...
        while self.page_history and (self.page.is_closed() or self.page not in self.context.pages):
            ...
            self.page = list(self.page_history.keys())[-1]  # set last active page as the active page (most recent)
    ...
    def _get_obs(self) -> dict:
        if self.use_raw_page_output:
            obs = {
                ...
                "goal_object": tuple(copy.deepcopy(self.goal_object)),  # new goal format, list of messages openai style
