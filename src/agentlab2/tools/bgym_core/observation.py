"""Observation extraction for BrowserGym."""
...
def _post_extract(page: playwright.sync_api.Page) -> None:
    js_frame_unmark_elements = pkgutil.get_data(__name__, "javascript/frame_unmark_elements.js").decode("utf-8")
...
def extract_data_items_from_aria(string: str, log_level: int = logging.NOTSET) -> tuple[list, str]:
    """
...
def pop_bids_from_attribute(dom_snapshot: dict, attr: str) -> None:
    try:
                ...
                for i in range(0, len(node_attributes), 2):
                    ...
                    if attr_name_id == target_attr_name_id:
                        ...
                        if attr_value_id not in processed_string_ids:
                            ...
                            dom_snapshot["strings"][attr_value_id] = new_attr_value  # update the string in the metadata
...
def extract_dom_extra_properties(dom_snapshot: dict, scale_factor: float, device_pixel_ratio: float = 1.0) -> dict:
    ...
    while docs_to_process:
        ...
        if parent:
            ...
            try:
                node_layout_idx = dom_snapshot["documents"][parent_doc]["layout"]["nodeIndex"].index(parent_node)
            except ValueError:
                node_layout_idx = -1
...
def extract_all_frame_axtrees(page: playwright.sync_api.Page) -> dict:
    """
...
def extract_merged_axtree(page: playwright.sync_api.Page) -> dict:
    """
        ...
        for node in ax_tree["nodes"]:
            if node["role"]["value"] == "Iframe":
                ...
                # if a frame is not found in the extracted AXTrees, we just ignore it
                    ...
                    logger.warning(f"AXTree merging: extracted AXTree does not contain frameId '{frame_id}', skipping")
...
() => {
    ...
    try:
        while frame:
            focused_element = frame.evaluate_handle(extract_focused_element_with_bid_script, BID_ATTR).as_element()
            if focused_element:
