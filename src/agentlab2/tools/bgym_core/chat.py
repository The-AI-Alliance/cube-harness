...
class Chat:
    def __init__(
        self, headless: bool, chat_size: tuple = (500, 800), record_video_dir: str | None = None, modern: bool = True
    ) -> None:
        self.messages = []

        # create a new browser, browser context and page for the chat
        pw: playwright.sync_api.Playwright = _get_global_playwright()
        self.browser = pw.chromium.launch(headless=headless, args=[f"--window-size={chat_size[0]},{chat_size[1]}"])
        ...
        # setup the chat page
        self.page.expose_function("send_user_message", lambda msg: self._js_user_message_received_callback(msg=msg))
    ...
    def _js_user_message_received_callback(self, msg: str) -> list:
    ...
    def add_message(self, role: Literal["user", "user_image", "assistant", "info", "infeasible"], msg: str) -> None:
