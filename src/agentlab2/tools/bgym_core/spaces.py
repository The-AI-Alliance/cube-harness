"""Gymnasium spaces for BrowserGym.
...
class AnyBox(Space[NDArray[Any]]):
    ...
    def __init__(self, low: int, high: int, shape: tuple, dtype: type) -> None:
    ...
    def __repr__(self) -> str:
        """Gives a string representation of this space."""
        return (
            f"AnyBox(low={repr(self.low)}, high={repr(self.high)}, shape={repr(self.shape)}, dtype={repr(self.dtype)})"
        )
