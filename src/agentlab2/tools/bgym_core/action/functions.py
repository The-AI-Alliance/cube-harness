"""BrowserGym action functions.

These are placeholders that define the action interface.
...
def fill(bid: str, value: str, enable_autocomplete_menu: bool = False) -> None:
    """
...
def select_option(bid: str, options: str | list[str]) -> None:
    """
...
def hover(bid: str) -> None:
    ...
    """
    elem = get_elem_by_bid(page, bid, demo_mode != "off")
    add_demo_mode_effects(page, elem, bid, demo_mode=demo_mode, move_cursor=True, highlight_box=False)
...
def scroll(delta_x: float, delta_y: float) -> None:
    ...
    Examples:
        ...
        scroll(-50.2, -100.5)
    """
    delta_x, delta_y = map_coordinates(page, delta_x, delta_y)  # map coordinates to page coordinates
    page.mouse.wheel(delta_x, delta_y)


def scroll_at(x: int, y: int, dx: int, dy: int) -> None:
    """
    Scroll horizontally and vertically. Amounts in pixels, positive for right or down scrolling, negative for left or up scrolling. Dispatches a wheel event.
...
def mouse_up(x: float, y: float, button: Literal["left", "middle", "right"] = "left") -> None:
    """
...
def mouse_down(x: float, y: float, button: Literal["left", "middle", "right"] = "left") -> None:
    """
...
def mouse_click(x: float, y: float, button: Literal["left", "middle", "right"] = "left") -> None:
    """
...
def mouse_dblclick(x: float, y: float, button: Literal["left", "middle", "right"] = "left") -> None:
    """
...
def mouse_drag_and_drop(from_x: float, from_y: float, to_x: float, to_y: float) -> None:
    """
...
def keyboard_insert_text(text: str) -> None:
    """
...
def upload_file(bid: str, file: str | list[str]) -> None:
    """
...
# https://playwright.dev/python/docs/input#upload-files
def mouse_upload_file(x: float, y: float, file: str | list[str]) -> None:
    """
