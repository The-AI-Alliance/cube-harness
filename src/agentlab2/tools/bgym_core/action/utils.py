...
def highlight_by_box(page: playwright.sync_api.Page, box: dict, color: Literal["blue", "red"] = "blue") -> None:
...
def smooth_move_visual_cursor_to(page: playwright.sync_api.Page, x: float, y: float, speed: float = 400) -> None:
    """
...
def call_fun(fun: callable, retry_with_force: bool) -> None:
    try:
        fun(force=False)
...
def map_coordinates(page: playwright.sync_api.Page, x: float, y: float) -> tuple[float, float]:
    """
